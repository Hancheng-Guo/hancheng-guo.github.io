...
from .validators import safe_local_svg_markup
...
def _local_svg_icon(path: str) -> str:
    """Inline one validated contact SVG for static and file:// previews."""
    markup = safe_local_svg_markup(path, root=Path(__file__).resolve().parents[1])
    if markup is None:
        return '<span class="svg-icon" aria-hidden="true"></span>'
    svg_start = re.search(r"<svg\b", markup, re.I)
    if not svg_start:
        return '<span class="svg-icon" aria-hidden="true"></span>'
    markup = markup[svg_start.start():]
    return re.sub(
        r"<svg\b",
        '<svg class="svg-icon svg-icon--inline" aria-hidden="true" focusable="false"',
        markup,
        count=1,
    )
...
def _contacts(portfolio: Any, prefix: str = "") -> str:
    ...
    for contact in portfolio.contacts:
        ...
        social.append(f'<div class="contact-item"><a href="{escape(str(contact.get("url", "")), quote=True)}" aria-label="{plain_label}" title="{plain_label}" target="_blank" rel="noopener noreferrer">{_local_svg_icon(str(contact.get("icon", "")))}<span class="visually-hidden">{label}</span></a></div>')
    if social:
