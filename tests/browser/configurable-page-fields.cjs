...
const site = {
  ...
  awards: [{ date: { start: '2022-01' }, title: { en: 'Award', zh: '奖项' }, description: { en: 'Award detail', zh: '奖项说明' } }], techStack: [],
...
(async () => {
  ...
  try {
    ...
    ok((await page.locator('.resume-awards .entry-detail').textContent()).includes('Award detail'), `Award description was not hydrated beside its date: ${errors.join(' | ')}`);
    await page.locator('.lang-toggle').click(); await page.waitForTimeout(100);
    ok((await page.locator('.resume-awards .entry-detail').textContent()).includes('奖项说明'), 'Localized award description was not rendered');
