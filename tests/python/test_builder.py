...
class BuilderTests(unittest.TestCase):
  ...
  def test_empty_cv_layout_disables_page_navigation_and_managed_cleanup(self):
    import tempfile
    for fields in (None, (), []):
      with self.subTest(fields=fields):
        portfolio = Portfolio()
        result = portfolio.set_cv_field() if fields is None else portfolio.set_cv_field(fields)
        self.assertIs(result, portfolio)
        self.assertEqual(portfolio.cv_fields, ())
        self.assertEqual(portfolio.site_document()["layout"]["cvFields"], [])
        portfolio.set_resume(label="Download PDF", url="assets/documents/CVTest.pdf")
        self.assertIn('href="assets/documents/CVTest.pdf"', render_home(portfolio))
        project = portfolio.add_project(title="Project", summary="Summary", thumbnail="assets/images/Avatar.jpg")
        project.add_page(template="minimal")
        for html in (render_home(portfolio), render_project(portfolio, project.data)):
          self.assertNotIn('data-i18n="nav.resume"', html)
        with tempfile.TemporaryDirectory() as folder:
          root = Path(folder)
          portfolio.write_static_fallbacks(root=root)
          self.assertFalse((root / "pages" / "cv.html").exists())

    enabled = Portfolio().set_cv_field(("profile",))
    self.assertEqual(enabled.cv_fields, ("profile",))
    self.assertIn('data-i18n="nav.resume"', render_home(enabled))
    self.assertIn('data-i18n="nav.resume">CV</a>', render_cv(enabled))
    with tempfile.TemporaryDirectory() as folder:
      root = Path(folder)
      enabled.write_static_fallbacks(root=root)
      cv = root / "pages" / "cv.html"
      self.assertIn("Generated by portfolio.py", cv.read_text(encoding="utf-8"))
      enabled.set_cv_field()
      enabled.write_static_fallbacks(root=root)
      self.assertFalse(cv.exists())
      cv.parent.mkdir(parents=True, exist_ok=True)
      cv.write_text("<html>hand-authored</html>", encoding="utf-8")
      enabled.write_static_fallbacks(root=root)
      self.assertEqual(cv.read_text(encoding="utf-8"), "<html>hand-authored</html>")
      enabled.set_cv_field(("education",))
      enabled.write_static_fallbacks(root=root)
      self.assertIn('data-i18n="nav.resume"', render_home(enabled))
      self.assertIn("Generated by portfolio.py", cv.read_text(encoding="utf-8"))
